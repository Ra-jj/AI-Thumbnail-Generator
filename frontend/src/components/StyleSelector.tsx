...
const StyleSelector = ({
  ...
  const styleDescriptions: Record<ThumbnailStyle, string> = {
    ...
    "Tech/Futuristic": "Modern, sleeek, tech-inspired",
  };
  ...
  return (
    <div className="relative space-y-3 dark">
      ...
      {isOpen && (
        <div className="absolute top-full z-50 mt-1 w-full rounded-md border border-white/12 bg-black/95 backdrop-blur-3xl shadow-lg">
          {thumbnailStyles.map((style) => (
            <button
              ...
              className={[
                "flex w-full items-start gap-3 px-4 py-3 text-left transition group",
                value === style
                  ? "bg-emerald-500/10 text-emerald-400"
                  : "text-zinc-200 hover:bg-white/5 hover:text-emerald-300"
              ].join(" ")}
            >
              <div className="mt-0.5 transition-colors group-hover:text-emerald-300">{styleIcons[style]}</div>
              <div>
                <p className="font-medium">{style}</p>
                <p className={[
                  "text-xs transition-colors",
                  value === style ? "text-emerald-500/70" : "text-zinc-400 group-hover:text-emerald-300/70"
                ].join(" ")}>
