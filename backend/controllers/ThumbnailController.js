...
export const generateThumbnail = async (req, res) => {
  try {
    ...
    // Simulate AI generation using a random placeholder image API (Picsum)
    // We use the prompt as a seed so the same prompt gives the same random image
    ...
    if (!imageResponse.ok) {
      throw new Error(
        `Failed to fetch placeholder image: ${imageResponse.statusText}`,
      );
    }
